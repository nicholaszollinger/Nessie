...
namespace nes
{
    ...
    class EPAPenetrationDepth
    {
        ...
        struct SupportPoints
        {
            ...
            Vec3    Add(const A& inA, const B& inB, const Vec3& direction, int& outIndex)
            {
                // Get the support point of the minkowski sum A - B
                Vec3 p = inA.GetSupport(direction);
                Vec3 q = inB.GetSupport(-direction);
                Vec3 w = p - q;
    ...
    public:
        ...
        //----------------------------------------------------------------------------------------------------
        template <ValidConvexObjectType A, ValidConvexObjectType B>
        EStatus         GetPenetrationDepthStepGJK(const A& inAExcludingConvexRadius, float convexRadiusA, const B& inBExcludingConvexRadius, float convexRadiusB, float tolerance, Vec3& ioV, Vec3& outPointA, Vec3& outPointB)
        {
        ...
        //----------------------------------------------------------------------------------------------------
        template <ValidConvexObjectType A, ValidConvexObjectType B>
        bool            GetPenetrationDepthStepEPA(const A& inAIncludingConvexRadius, const B& inBIncludingConvexRadius, float tolerance, Vec3& outV, Vec3& outPointA, Vec3& outPointB)
        {
            ...
            switch (supportPoints.m_y.size())
            {
                case 1:
                {
                    ...
                    int p1, p2, p3, p4;
                    (void)supportPoints.Add(inAIncludingConvexRadius, inBIncludingConvexRadius, Vec3(0.f, 1.f, 0.f), p1);
                    (void)supportPoints.Add(inAIncludingConvexRadius, inBIncludingConvexRadius, Vec3(-1.f, -1.f, -1.f), p2);
                    (void)supportPoints.Add(inAIncludingConvexRadius, inBIncludingConvexRadius, Vec3(1.f, -1.f, -1.f), p3);
                    (void)supportPoints.Add(inAIncludingConvexRadius, inBIncludingConvexRadius, Vec3(0.f, -1.f, 1.f), p4);
                    NES_ASSERT(p1 == 0);
                    NES_ASSERT(p2 == 1);
                    NES_ASSERT(p3 == 2);
                    NES_ASSERT(p4 == 3);
                ...
                case 2:
                {
                    ...
                    Vec3 dir2 = rotation.TransformPoint(dir1);
                    Vec3 dir3 = rotation.TransformPoint(dir2);
            ...
            for (;;)
            {
                ...
                int newIndex;
                Vec3 w = supportPoints.Add(inAIncludingConvexRadius, inBIncludingConvexRadius, pTri->m_normal, newIndex);
            ...
            do
            {
                ...
                float distSqr = math::Squared(dot) / pTri->m_normal.LengthSqr();
                ...
                if (hasDefect)
                {
                    ...
                    Vec3 w2 = inAIncludingConvexRadius.GetSupport(-pTri->m_normal) - inBIncludingConvexRadius.GetSupport(pTri->m_normal);
            ...
            outV = (pLast->m_centroid.Dot(pLast->m_normal) / pLast->m_normal.LengthSqr()) * pLast->m_normal;
            ...
            Vec3 p0 = supportPoints.m_p[pLast->m_edges[0].m_startIndex];
            Vec3 p1 = supportPoints.m_p[pLast->m_edges[1].m_startIndex];
            Vec3 p2 = supportPoints.m_p[pLast->m_edges[2].m_startIndex];

            Vec3 q0 = supportPoints.m_q[pLast->m_edges[0].m_startIndex];
            Vec3 q1 = supportPoints.m_q[pLast->m_edges[1].m_startIndex];
            Vec3 q2 = supportPoints.m_q[pLast->m_edges[2].m_startIndex];
        ...
        bool            GetPenetrationDepth(const AE& inAExcludingConvexRadius, const AI& inAIncludingConvexRadius, float convexRadiusA, const BE& inBExcludingConvexRadius, const BI& inBIncludingConvexRadius, float convexRadiusB, float collisionToleranceSq, float penetrationTolerance, Vec3& ioV, Vec3& outPointA, Vec3& outPointB)
        {
        ...
        //----------------------------------------------------------------------------------------------------
        template <ValidConvexObjectType A, ValidConvexObjectType B>
        bool            CastShape(const Mat44& start, const Vec3& direction, float collisionTolerance, float penetrationTolerance, const A& inA, const B& inB, float convexRadiusA, float convexRadiusB, bool returnDeepestPoint, float& ioLambda, Vec3& outPointA, Vec3& outPointB, Vec3& outContactNormal)
        {
