...
namespace nes
{
    DeviceImage::~DeviceImage()
    {
        if (m_ownsNativeObjects)
        {
            if (m_allocation)
            {
                // [TODO]: Needs to be Renderer::SubmitResourceFree();
    ...
    EGraphicsResult DeviceImage::Init(const VkImage image, const TextureDesc& textureDesc)
    {
    ...
    EGraphicsResult DeviceImage::Init(const AllocateTextureDesc& textureDesc)
    {
        NES_ASSERT(m_handle == nullptr);
    ...
    void DeviceImage::SetDebugName(const char* name)
    {
