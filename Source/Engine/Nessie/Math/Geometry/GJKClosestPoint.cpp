...
namespace nes
{
    ...
    {
        float yLengthSqr = m_y[0].SquaredMagnitude();
        for (int i = 1; i < m_numPoints; ++i)
            yLengthSqr = math::Max(yLengthSqr, m_y[i].SquaredMagnitude());
    ...
    void GJKClosestPoint::CalculatePointAAndB(Vector3& outPointA, Vector3& outPointB) const
    {
        switch (m_numPoints)
        {
            case 1:
    ...
    void GJKClosestPoint::GetClosestPointsSimplex(Vector3* pOutY, Vector3* pOutP, Vector3* pOutQ, size_t& outNumPoints) const
    {
        unsigned size = sizeof(Vector3) * m_numPoints;
