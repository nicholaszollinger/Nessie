...
namespace nes
{
    ...
    EPAConvexHullBuilder::Triangle::Triangle(int index0, int index1, int index2, const Vector3* pPositions)
    {
        // Fill in indexes
        ...
        Vector3 v0 = pPositions[index0];
        Vector3 v1 = pPositions[index1];
        Vector3 v2 = pPositions[index2];
        ...
        if (e20Dote20 < e21Dote21)
        {
            ...
            const float normalLenSqr = m_normal.SquaredMagnitude();
            if (normalLenSqr > kMinTriangleArea)
            {
                ...
                const float e10Dote10 = e10.SquaredMagnitude();
        ...
        else
        {
            ...
            float normalLenSqr = m_normal.SquaredMagnitude();
            if (normalLenSqr > kMinTriangleArea)
            {
    ...
    EPAConvexHullBuilder::Triangle* EPAConvexHullBuilder::TriangleFactory::CreateTriangle(int index0, int index1,
        int index2, const Vector3* pPositions)
    {
        Triangle* pResult;
    ...
    EPAConvexHullBuilder::Triangle* EPAConvexHullBuilder::FindFacingTriangle(const Vector3& position,
        float& outBestDistSqr)
    {
        ...
        for (Triangle* pTri : m_queue)
        {
            if (!pTri->m_isRemoved)
            {
                ...
                if (dot > 0.f)
                {
                    const float distSqr = dot * dot / pTri->m_normal.SquaredMagnitude();
    ...
    bool EPAConvexHullBuilder::AddPoint(Triangle* pFacingTriangle, int index, float closestDistSqr, NewTriangles& outTriangles)
    {
        ...
        Vector3 position = m_points[index];
    ...
    bool EPAConvexHullBuilder::FindEdge(Triangle* pFacingTriangle, const Vector3& vertex, Edges& outEdges)
    {
