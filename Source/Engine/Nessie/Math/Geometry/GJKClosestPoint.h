...
namespace nes
{
    ...
    {
        Vector3 m_y[4];             /// Support points on A - B
        Vector3 m_p[4];             /// Support point on A
        Vector3 m_q[4];             /// Support point on B
    ...
    public:
        ...
        bool            Intersects(const A& inA, const B& inB, float tolerance, Vector3& separatingAxis);
        ...
        //----------------------------------------------------------------------------------------------------
        template <ValidConvexObjectType A, ValidConvexObjectType B>
        float           GetClosestPoints(const A& inA, const B& inB, const float tolerance, const float maxDistSqr, Vector3& separatingAxis, Vector3& outPointA, Vector3& outPointB);
        ...
        //----------------------------------------------------------------------------------------------------
        void            GetClosestPointsSimplex(Vector3* pOutY, Vector3* pOutP, Vector3* pOutQ, size_t& outNumPoints) const;
        ...
        bool            CastRay(const Vector3& rayOrigin, const Vector3& rayDirection, float tolerance, const A& inA, float& ioLambda);
        ...
        //----------------------------------------------------------------------------------------------------
        template <ValidConvexObjectType A, ValidConvexObjectType B>
        bool            CastShape(const Mat4& start, const Vector3& direction, float tolerance, const A& inA, const B& inB, float& ioLambda);
        ...
        //----------------------------------------------------------------------------------------------------
        template <ValidConvexObjectType A, ValidConvexObjectType B>
        bool            CastShape(const Mat4& start, const Vector3& direction, float tolerance, const A& inA, const B& inB, float convexRadiusA, float convexRadiusB, float& ioLambda, Vector3& outPointA, Vector3& outPointB, Vector3& outSeparatingAxis);
    ...
    private:
        ...
        bool            GetClosest(const float prevVecLenSqr, Vector3& outVec, float& outVecLenSqr, uint32_t &outSet) const;
        ...
        //----------------------------------------------------------------------------------------------------
        void            CalculatePointAAndB(Vector3& outPointA, Vector3& outPointB) const;
    };

    template <ValidConvexObjectType A, ValidConvexObjectType B>
    bool GJKClosestPoint::Intersects(const A& inA, const B& inB, float tolerance, Vector3& separatingAxis)
    {
        ...
        for (;;)
        {
            ...
            Vector3 p = inA.GetSupport(separatingAxis);
            Vector3 q = inB.GetSupport(-separatingAxis);
    ...
    float GJKClosestPoint::GetClosestPoints(const A& inA, const B& inB, const float tolerance, const float maxDistSqr,
        Vector3& separatingAxis, Vector3& outPointA, Vector3& outPointB)
    {
        // Note: 'separatingAxis' == 'v' for comments in this function, because it is interpreted as just a direction in
        // portions of the algorithm.
        ...
        // Length^2 of v
        float vLengthSqr = separatingAxis.SquaredMagnitude();
        ...
        NES_ASSERT(separatingAxis.SquaredMagnitude() == vLengthSqr);
        return vLengthSqr;
    }

    template <ValidConvexObjectType A>
    bool GJKClosestPoint::CastRay(const Vector3& rayOrigin, const Vector3& rayDirection, const float tolerance, const A& inA, float& ioLambda)
    {
        float toleranceSqr = tolerance * tolerance;
        ...
        float lambda = 0.f;
        Vector3 x = rayOrigin;
        Vector3 v = x - inA.GetSupport(Vector3::Zero());
        float vLengthSqr = v.SquaredMagnitude();
        bool allowRestart = false;

        for (;;)
        {
            // Get new support point
            Vector3 p = inA.GetSupport(v);
            Vector3 w = x - p;
    ...
    template <ValidConvexObjectType A, ValidConvexObjectType B>
    bool GJKClosestPoint::CastShape(const Mat4& start, const Vector3& direction, float tolerance, const A& inA, const B& inB, float& ioLambda)
    {
        ...
        return CastRay(Vector3::Zero(), direction, tolerance, difference, ioLambda);
    }

    template <ValidConvexObjectType A, ValidConvexObjectType B>
    bool GJKClosestPoint::CastShape(const Mat4& start, const Vector3& direction, float tolerance, const A& inA,
        const B& inB, float convexRadiusA, float convexRadiusB, float& ioLambda, Vector3& outPointA, Vector3& outPointB,
        Vector3& outSeparatingAxis)
    {
        float toleranceSqr = tolerance * tolerance;
        ...
        float lambda = 0.f;
        Vector3 x = Vector3::Zero(); // Since A is already transformed we can start the cast from zero.

        // See CastRay: v = x - inA.GetSupport(Vector3::Zero()) where inA is the Minkowski difference inB - transformedA (see CastShape above) and x is zero
        Vector3 v = -inB.GetSupport(Vector3::Zero()) + transformedA.GetSupport(Vector3::Zero());
        ...
        for (;;)
        {
            ...
            Vector3 p = transformedA.GetSupport(-v);
            Vector3 q = inB.GetSupport(v);
            Vector3 w = x - (q - p);
            ...
            float vDotW = v.Dot(w) - sumConvexRadius * v.Magnitude();
        ...
        const Vector3 normalizedV = v.NormalizedOr(Vector3::Zero());
        const Vector3 vecConvexRadiusA = convexRadiusA * normalizedV; 
        const Vector3 vecConvexRadiusB = convexRadiusB * normalizedV;
    ...
    template <bool LastPointPartOfClosestFeature>
    bool GJKClosestPoint::GetClosest(const float prevVecLenSqr, Vector3& outVec, float& outVecLenSqr,
        uint32_t& outSet) const
    {
        ...
        const float vecLenSqr = vec.SquaredMagnitude();
