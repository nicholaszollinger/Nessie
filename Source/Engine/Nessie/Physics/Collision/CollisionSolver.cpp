...
namespace nes
{
    ...
    void CollisionSolver::Internal_Init()
    {
        ...
        for (size_t i = 0; i < kNumSubShapeTypes; ++i)
        {
            for (size_t j = 0; j < kNumSubShapeTypes; ++j)
            {
                ...
                {
                    s_collideShapeFunctions[i][j] = [](const Shape*, const Shape*, const Vec3&, const Vec3&, const Mat44&, const Mat44&, const SubShapeIDCreator&, const SubShapeIDCreator&, const CollideShapeSettings&, CollideShapeCollector&, const ShapeFilter&)
                    {
                ...
                {
                    s_castShapeFunctions[i][j] = [](const ShapeCast&, const ShapeCastSettings&, const Shape*, const Vec3&, const ShapeFilter&, const Mat44&, const SubShapeIDCreator&, const SubShapeIDCreator&, CastShapeCollector&)
                    {
    ...
    void CollisionSolver::ReversedCollideShape(const Shape* pShape1, const Shape* pShape2, const Vec3& scale1,
    ...
    void CollisionSolver::ReversedCastShape(const ShapeCast& shapeCast, const ShapeCastSettings& shapeCastSettings,
        const Shape* pShape, const Vec3& scale, const ShapeFilter& shapeFilter, const Mat44& centerOfMassTransform2,
        const SubShapeIDCreator& subShapeIDCreator1, const SubShapeIDCreator& subShapeIDCreator2,
        CastShapeCollector& collector)
    {
        ...
        public:
            explicit ReversedCollector(CastShapeCollector& collector, const Vec3& worldDirection)
                : CastShapeCollector(collector)
                , m_collector(collector)
        ...
        Mat44 comStartInverse = shapeCast.m_centerOfMassStart.InversedRotationTranslation();
        ...
        const Mat44 shape1COM = centerOfMassTransform2 * shapeCast.m_centerOfMassStart;

        // Calculate the world space direction vector of the shape cast
        const Vec3 worldDirection = -centerOfMassTransform2.TransformVector(shapeCast.m_direction);
